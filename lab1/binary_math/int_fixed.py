...
def decimal_to_binary_unsigned(n, bits=BITS31):
...
def direct_code(n):
    ...
    if n < 0:
        result[0] = 1
        n = abs(n)
    result[1:] = decimal_to_binary_unsigned(n, BITS31)
    return result
...
def subtract_twos_complement(a, b):
    return add_twos_complement(a, b)
...
def multiply_direct(a_arr, b_arr):
    ...
    result = np.zeros(BITS31, dtype=int)

    for i in range(BITS31 - 1, -1, -1):
        if b[i] == 1:
            shift = (BITS31 - 1) - i

            temp = np.zeros(BITS31, dtype=int)

            if shift == 0:
                temp[:] = a
            else:
                temp[:BITS31 - shift] = a[shift:]

            carry = 0
            for j in range(BITS31 - 1, -1, -1):
                s = result[j] + temp[j] + carry
                result[j] = s % 2
                carry = s // 2

    final = np.zeros(BITS32, dtype=int)
    final[0] = sign
    final[1:] = result
...
def divide_direct(a_arr, b_arr):
    ...
    remainder = np.zeros(BITS31, dtype=int)
    quotient = np.zeros(BITS31, dtype=int)

    for i in range(BITS31 + FRAC_BITS):
